...
enum DiskExplorerTrashPolicy {
    /// True when `path` is Home itself or anything under it (symlinks in the
    /// parent chain resolved). Used for the read-only banner.
    static func isInsideHome(_ path: String, home: String = NSHomeDirectory()) -> Bool {
