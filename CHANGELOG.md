...
- **CI now fails on new compiler warnings under `Sources/`.** Swift 6 actor-isolation diagnostics are warnings, not errors, so a reverted `Sendable` conformance could regress with the full 2459-test suite still green. `Scripts/check-source-warnings.sh` builds the package and fails on any warning whose file path is under this repo's own `Sources/`; it runs as the test job's first build, ahead of `Test with coverage`. The gate is scoped there rather than package-wide because `BuildMetallibPlugin.swift` carries 8 pre-existing `Path` -> `URL` deprecations that are their own migration, and scoping to the repo root also keeps dependency warnings under `.build/checkouts/*/Sources/` out of it. Three `String(cString:)` deprecations the gate surfaced under `Sources/` are fixed.
